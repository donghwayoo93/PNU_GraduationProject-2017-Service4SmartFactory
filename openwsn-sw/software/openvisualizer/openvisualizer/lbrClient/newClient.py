...
import logging
log = logging.getLogger('newClient')
...
class lbrClient(threading.Thread):
    ...
    def __init__(self):
        ...
        # log
        log.info("creating newClient instance")
    ...
    def run(self):
        try:
            # log
            log.info("starting to run newClient")
            ...
            while True:
                ...
                # log
                log.info("starting to listen for data newClient")
                    ...
                    while True:
                        ...
                        if not input:
                            if self._isConnected():
                                self.disconnect("No input. at newClient")
                            break
                        ...
                        if len(input)<8:
                            log.error("newClient received packet from LBR which is too short ({0} bytes)".format(len(input)))
                            continue
                        
                        # dispatch the packet to network state to figure out source route.
                        print 'it is from new client data : '
                        print input
                        print '\n'
        ...
        try:
            if self._isConnected():
                ...
                if log.isEnabledFor(logging.DEBUG):
                    log.debug('packet sent to newClient: {}'.format(printlowpan))
