...
/*
		for (idx = 0; idx < 16; idx++) {
			sourceAddr[idx] = (uint8_t)msg->l3_sourceAdd.addr_128b[idx];
			destinationAddr[idx] = (uint8_t)msg->l3_destinationAdd.addr_128b[idx];
		}

		strcpy(src_str, sourceAddr);
		strcpy(dest_str, destinationAddr);

		// source addr
		packetfunctions_reserveHeaderSize(msg, 1);
		msg->payload[0] = '\n';
		packetfunctions_reserveHeaderSize(msg, sizeof(sourceAddr));
		memcpy(&msg->payload[0], &sourceAddr, sizeof(sourceAddr));

		split[16] = "111111111111111";
		// splitter
		packetfunctions_reserveHeaderSize(msg, 1);
		msg->payload[0] = '\n';
		packetfunctions_reserveHeaderSize(msg, sizeof(split) - 1);
		memcpy(&msg->payload[0], &split, sizeof(split) - 1);

		//destination addr
		packetfunctions_reserveHeaderSize(msg, 1);
		msg->payload[0] = '\n';
		packetfunctions_reserveHeaderSize(msg, sizeof(destinationAddr));
		memcpy(&msg->payload[0], &destinationAddr, sizeof(destinationAddr));
*/
		/**********************************************/
