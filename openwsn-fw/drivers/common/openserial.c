...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/* used by cinst (user added part) */

owerror_t openserial_printInst(uint8_t* buffer, uint8_t length) {
    uint8_t  i;
    uint8_t  asn[5];
    INTERRUPT_DECLARATION();
    ...
    openserial_vars.outputBufFilled  = TRUE;
    outputHdlcOpen();
    outputHdlcWrite(SERFRAME_MOTE2PC_MANUAL);
    outputHdlcWrite(idmanager_getMyID(ADDR_16B)->addr_16b[1]);
    outputHdlcWrite(idmanager_getMyID(ADDR_16B)->addr_16b[0]);
    outputHdlcWrite(asn[0]);
    outputHdlcWrite(asn[1]);
    ...
    if (busyReceiving==FALSE && inputBufFill>0) {
        ...
        switch (cmdByte) {
            ...
            case SERFRAME_PC2MOTE_COMMAND:
                ...
                break;
			case SERFRAME_PC2MOTE_RESPONSE:
				cinst_task_cb();
				break;
        }
